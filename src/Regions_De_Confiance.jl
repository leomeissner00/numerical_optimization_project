...
function Regions_De_Confiance(algo,f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    #Verification des conditions d'arret avant la premiere iteration
    ...
    while flag == -1
        ...
        if algo == "gct"
            s = Gradient_Conjugue_Tronque(gradf(xmin),hessf(xmin),[deltak;max_iter;Tol_rel])
        elseif algo == "cauchy"
            s, _ = Pas_De_Cauchy(gradf(xmin),hessf(xmin),deltak)
        end

        #Calcul de mk
        m = f(xmin)
        ms = f(xmin) + gradf(xmin)'*s +0.5*s'*hessf(xmin)*s

        rhok = (f(xmin) - f(xmin+s))/(m - ms)

        xk = xmin
        if rhok >= eta1
            xmin = xk + s     #Mise à jour de l'itéré
        end

        if rhok >= eta2
            deltak = min(gamma2*deltak, deltaMax)   #On augmente la région de confiance
        elseif rhok >= eta1
            deltak = deltak
        else
            deltak = gamma1*deltak # On diminue la région de confiance
        end
        ...
        nb_iters = nb_iters + 1;
            if norm(gradf(xmin)) <= max(Tol_rel * norm(gradf(x0)),Tol_abs) 
                flag = 0;
            elseif norm(xmin - xk) <= max(Tol_rel * norm(xmin - xk),Tol_abs) && rhok >= eta1
                    flag = 1;     
            elseif abs(f(xmin) - f(xk)) <= epsilon * max(Tol_rel*abs(f(xk)),Tol_abs) && rhok >= eta1
                    flag = 2;
            elseif nb_iters >= max_iter
                flag = 3;
            end
