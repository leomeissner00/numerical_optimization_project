...
function Algorithme_De_Newton(f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    while flag == 10
        ...
        #Calcul de ma direction de Newton
        dk = hessf(xmin)\(-gradf(xmin))

        #Mise a jour de l'itere
        xmin = xmin + dk
        nb_iters = nb_iters + 1;
        
        #Calcul des criteres d'arret
        if norm(gradf(xmin)) <= max(Tol_rel * norm(gradf(x0)),Tol_abs) 
            flag = 0;
        
        elseif norm(xmin - xk) <= max(Tol_rel * norm(xmin - xk),Tol_abs)
            flag = 1;
